...
export class ImageEditorManager {
  ...
  private dragHandle: CropHandle = 'none';
  private dragStartPos: Point = { x: 0, y: 0 };
  private dragStartQuad: Quad | null = null;

  /** Filename to send back; also tells the user what they are editing. */
  private filename = 'edited.jpg';
  ...
  init(): void {
    this.canvas = document.getElementById('editorCanvas') as HTMLCanvasElement | null;
    ...
    const brightness = document.getElementById('editorBrightness') as HTMLInputElement | null;
    const contrast = document.getElementById('editorContrast') as HTMLInputElement | null;
  ...
  async openForDocument(docId: number, title: string): Promise<void> {
  ...
  async openForFile(file: File): Promise<void> {
    this.filename = file.name;
    await this.openWithSource(URL.createObjectURL(file), true);
  }

  private async openWithSource(src: string, revoke = false): Promise<void> {
  ...
  private render(): void {
    if (!this.ctx || !this.canvas || !this.image || !this.quad) return;
  ...
  private eventPos(e: PointerEvent): Point {
  ...
  private onPointerDown(e: PointerEvent): void {
    ...
    if (this.dragHandle === 'none') return;
  ...
  private onPointerMove(e: PointerEvent): void {
    if (!this.quad || !this.image) return;
    const pos = this.eventPos(e);
  ...
  private resetAdjust(): void {
    this.brightness = 0;
    this.contrast = 0;
    const b = document.getElementById('editorBrightness') as HTMLInputElement | null;
    const c = document.getElementById('editorContrast') as HTMLInputElement | null;
    if (b) b.value = '0';
    if (c) c.value = '0';
  ...
  private async apply(): Promise<void> {
    if (!this.image || !this.quad) return;
    const button = document.getElementById('btnEditorApply') as HTMLButtonElement | null;
    ...
    const blob: Blob | null = await new Promise(resolve => out.toBlob(resolve, 'image/jpeg', 0.92));
    if (!blob) {
...
(window as any).ImageEditorManager = ImageEditorManager;
