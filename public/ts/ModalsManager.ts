...
class ModalsManager {
  ...
  setupEventListeners(): void {
    ...
    document.getElementById('relocalizeCategorySelect')?.addEventListener('change', (e: any) => {
      this.updateSubcategoriesDropdown(e.target.value);
      this.combineRelocalizeReasons();
    });
    document.getElementById('relocalizeSubcategorySelect')?.addEventListener('change', (e: any) => {
      const customInput = document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement | null;
      if (customInput) {
    ...
    document.getElementById('btnDownloadMarkdown')?.addEventListener('click', () => {
      ...
      if (doc && doc.id) window.location.href = `/api/documents/${doc.id}/markdown`;
    });
    const grandModal = document.getElementById('grandViewerModal');
    if (grandModal) {
      grandModal.addEventListener('click', e => {
        if (e.target === grandModal) this.closeGrandViewerModal();
      });
    }
    ...
    document.getElementById('cfgOllamaModelSelect')?.addEventListener('change', (e: any) => {
  ...
  switchLogsTab(tab: string): void {
    ...
    if (tab === 'stream') {
      ...
      if (btnStream) btnStream.style.borderBottom = '2px solid #38bdf8';
      if (btnSessions) btnSessions.style.borderBottom = 'none';
    } else {
      if (streamView) streamView.style.display = 'none';
      if (sessionsView) sessionsView.style.display = 'flex';
      if (btnStream) btnStream.style.borderBottom = 'none';
      if (btnSessions) btnSessions.style.borderBottom = '2px solid #38bdf8';
    }
    this.loadLogsView();
  }

  async loadLogsView(): Promise<void> {
    ...
    if (state.currentLogsTab === 'stream') {
      try {
        ...
        if (container && Array.isArray(data.logs)) {
          container.innerHTML = data.logs.map((entry: any) => this.formatLogLine(entry)).join('');
      ...
      try {
        ...
        if (container && Array.isArray(data.sessions)) {
          ...
          container.innerHTML = data.sessions.map((sess: any, idx: number) => {
            ...
            const logLines = (sess.logs || []).map((entry: any) => this.formatLogLine(entry)).join('');
  ...
  toggleSessionDetails(idx: number): void {
  ...
  formatLogLine(entry: any): string {
    const state = this.app.state;
    let levelColor = '#94a3b8';
    if (entry.level === 'INFO') levelColor = '#34d399';
    if (entry.level === 'WARN') levelColor = '#fbbf24';
    if (entry.level === 'ERROR') levelColor = '#f87171';
    if (entry.level === 'DEBUG') levelColor = '#38bdf8';
  ...
  setupLogsSSE(): void {
    const state = this.app.state;
    if (!(window as any).EventSource || state.logsSseConnected) return;
  ...
  /* --- PDF TOOL MODAL --- */
  openPdfUtilModal(): void {
    const modal = document.getElementById('pdfUtilModal');
    if (modal) modal.classList.add('open');
  }

  closePdfUtilModal(): void {
    const modal = document.getElementById('pdfUtilModal');
    if (modal) modal.classList.remove('open');
  }

  switchPdfUtilTab(tab: string): void {
    ...
    if (tab === 'split') {
      if (splitTab) splitTab.style.display = 'block';
      if (mergeTab) mergeTab.style.display = 'none';
      if (btnSplit) btnSplit.style.borderBottom = '2px solid var(--accent-blue)';
      if (btnMerge) btnMerge.style.borderBottom = 'none';
    } else {
      if (splitTab) splitTab.style.display = 'none';
      if (mergeTab) mergeTab.style.display = 'block';
      if (btnSplit) btnSplit.style.borderBottom = 'none';
      if (btnMerge) btnMerge.style.borderBottom = '2px solid var(--accent-blue)';
    }
  }

  async handlePdfSplit(e: any): Promise<void> {
    e.preventDefault();
    const state = this.app.state;
  ...
  async handlePdfMerge(e: any): Promise<void> {
    e.preventDefault();
    const state = this.app.state;
  ...
  /* --- BLOCKED FILES MODAL --- */
  async updateBlockedFilesBadge(): Promise<void> {
    try {
  ...
  async openBlockedFilesModal(): Promise<void> {
    const state = this.app.state;
    ...
    try {
      ...
      listEl.innerHTML = data.files.map((f: any) => `
  ...
  setupBlockedFilesModal(): void {
    ...
    modal.addEventListener('click', e => {
      if (e.target === modal) this.closeBlockedFilesModal();
    });
  }

  /* --- GRAND VIEWER MODAL --- */
  switchGrandViewerTab(tab: string): void {
    const state = this.app.state;
    ...
    if (tab === 'markdown') {
      if (btnMd) btnMd.style.borderBottom = '2px solid var(--accent-blue)';
      if (btnRaw) btnRaw.style.borderBottom = 'none';
    } else {
      if (btnMd) btnMd.style.borderBottom = 'none';
      if (btnRaw) btnRaw.style.borderBottom = '2px solid var(--accent-blue)';
    }
  ...
  updateGrandViewerTextDisplay(): void {
    const state = this.app.state;
    if (!state.currentGrandViewerDoc) return;
  ...
  async openGrandViewerModal(docId: number, event?: any): Promise<void> {
    const state = this.app.state;
    if (event) {
    ...
    try {
      ...
      if (sumEl) sumEl.innerHTML = state.renderMarkdown(doc.summary || 'No summary available.');

      const contactNameVal = (doc.contact_name || '').trim();
      const contactNameEl = document.getElementById('grandViewerContactName');
      if (contactNameEl) contactNameEl.textContent = contactNameVal || '-';

      const contactEmailEl = document.getElementById('grandViewerContactEmail');
      if (contactEmailEl) contactEmailEl.textContent = doc.contact_email || '-';

      const contactPhoneEl = document.getElementById('grandViewerContactPhone');
      if (contactPhoneEl) contactPhoneEl.textContent = doc.contact_phone || '-';

      const contactAddrEl = document.getElementById('grandViewerContactAddress');
      if (contactAddrEl) contactAddrEl.textContent = doc.contact_address || '-';

      const contactWebEl = document.getElementById('grandViewerContactWebsite');
      if (contactWebEl) contactWebEl.textContent = doc.contact_website || '-';
      ...
      if (btnChrome) btnChrome.onclick = () => this.app.events.openInChrome(doc.id);

      const btnLoc = document.getElementById('btnGrandOpenLocation');
      if (btnLoc) btnLoc.onclick = () => this.app.events.openFileLocation(targetPath);

      const btnReanalyze = document.getElementById('btnGrandReanalyze') as HTMLButtonElement | null;
      if (btnReanalyze) {
  ...
  /* --- RELOCALIZE MODAL --- */
  async openRelocalizeModal(docId: number): Promise<void> {
    const state = this.app.state;
    try {
      const res = await fetch(`/api/documents/${docId}`);
      const doc = await res.json();
      ...
      (document.getElementById('relocalizeDocId') as HTMLInputElement).value = doc.id.toString();
      (document.getElementById('relocalizeDocTitle') as HTMLElement).textContent = doc.title;
      (document.getElementById('relocalizeCurrentPath') as HTMLElement).textContent = `Current Path: ${doc.new_path || doc.original_path}`;
      const reasonTextarea = document.getElementById('relocalizeReason') as HTMLTextAreaElement | null;
      if (reasonTextarea) {
        reasonTextarea.value = '';
        reasonTextarea.placeholder = 'Explain what local AI got wrong so it learns for future runs...';
      }
      (document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement).value = '';
      (document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement).style.display = 'none';

      const catList = ((window as any).categories && (window as any).categories.length > 0) ? (window as any).categories : (state.categories || []);
      const catSelect = document.getElementById('relocalizeCategorySelect') as HTMLSelectElement;
      catSelect.innerHTML = catList.map((c: any) => 
        `<option value="${c.id}" ${c.id === doc.category ? 'selected' : ''}>${state.getLocalizedName(c)} (${c.id})</option>`
      ).join('');
  ...
  updateSubcategoriesDropdown(selectedCatId: string, selectedSubId?: string): void {
    const state = this.app.state;
    const catList = ((window as any).categories && (window as any).categories.length > 0) ? (window as any).categories : (state.categories || []);
    const catObj = catList.find((c: any) => c.id === selectedCatId);
    const subSelect = document.getElementById('relocalizeSubcategorySelect') as HTMLSelectElement;
    const allSubs = (catObj && catObj.subcategories) ? catObj.subcategories : [];
    
    const subs = allSubs
      .filter((s: any) => !state.isForbiddenSubcategory(s.id))
      .sort((a: any, b: any) => state.getLocalizedName(a).localeCompare(state.getLocalizedName(b), undefined, { sensitivity: 'base' }));

    let optionsHtml = subs.map((s: any) => 
      `<option value="${s.id}" ${s.id === selectedSubId ? 'selected' : ''}>${state.getLocalizedName(s)} (${s.id})</option>`
    ).join('');
    ...
    const customInput = document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement;
  ...
  combineRelocalizeReasons(): void {
    const catSelect = document.getElementById('relocalizeCatErrorReason') as HTMLSelectElement | null;
    const subSelect = document.getElementById('relocalizeSubErrorReason') as HTMLSelectElement | null;
    ...
    if (parts.length > 0) {
      ...
      const catTarget = (document.getElementById('relocalizeCategorySelect') as HTMLSelectElement | null)?.value || '';
      const subRaw = (document.getElementById('relocalizeSubcategorySelect') as HTMLSelectElement | null)?.value || '';
      const subTarget = (subRaw === '__NEW__' || subRaw === '__EDIT__')
        ? ((document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement | null)?.value || '').trim()
        : subRaw;
      ...
      if (changedCat && changedSub) parts.push(`Target: ${catTarget}/${subTarget}`);
      else if (changedCat) parts.push(`Target: ${catTarget}/${doc.subcategory || ''}`);
      else if (changedSub) parts.push(`Target: ${catTarget || doc.category || ''}/${subTarget}`);
    }

    const textarea = document.getElementById('relocalizeReason') as HTMLTextAreaElement | null;
    if (textarea) {
  ...
  async handleConfirmRelocalize(): Promise<void> {
    const state = this.app.state;
    const docId = (document.getElementById('relocalizeDocId') as HTMLInputElement).value;
    const category = (document.getElementById('relocalizeCategorySelect') as HTMLSelectElement).value;
    let subcategory = (document.getElementById('relocalizeSubcategorySelect') as HTMLSelectElement).value;
    const customSub = (document.getElementById('relocalizeCustomSubcategory') as HTMLInputElement).value.trim();
  ...
  async handleAiReanalyze(): Promise<void> {
    const docId = (document.getElementById('relocalizeDocId') as HTMLInputElement).value;
    const reason = (document.getElementById('relocalizeReason') as HTMLTextAreaElement).value.trim();
    const state = this.app.state;
  ...
  /* --- EDIT METADATA MODAL --- */
  async openEditModal(docId: number): Promise<void> {
    const state = this.app.state;
    try {
      const res = await fetch(`/api/documents/${docId}`);
      const doc = await res.json();
      ...
      if (subtitle) subtitle.textContent = `ID #${doc.id} · Created ${doc.created_at ? doc.created_at.substring(0,10) : '?'}`;

      const infoFilename = document.getElementById('editInfoFilename');
      if (infoFilename) infoFilename.textContent = `📄 ${doc.original_filename || doc.title}`;

      const infoStatus = document.getElementById('editInfoStatus');
      if (infoStatus) {
        const statusColor: any = { MOVED: '#4ade80', PENDING: '#facc15', FAILED: '#f87171', SKIPPED: '#94a3b8' };
      ...
      const rawTextEl = document.getElementById('editRawText') as HTMLTextAreaElement | null;
      if (rawTextEl) {
        rawTextEl.value = doc.markdown_content || doc.raw_text || '[No text content extracted]';
      }

      // Set char count for summary
      const sumEl = document.getElementById('editSummary') as HTMLTextAreaElement | null;
      const countEl = document.getElementById('editSummaryCount');
      if (sumEl && countEl) countEl.textContent = sumEl.value.length + ' chars';

      // Ensure raw text section starts collapsed
      const rawSection = rawTextEl?.closest('.edit-section');
      if (rawSection) rawSection.classList.add('edit-section-collapsed');
  ...
  async handleSaveEdit(e: any): Promise<void> {
    e.preventDefault();
    const state = this.app.state;
    ...
    const updates = {
      ...
      tags: state.getVal('editTags').split(',').map((t: string) => t.trim()).filter(Boolean)
    };
  ...
  async loadOllamaModels(activeModel?: string): Promise<void> {
    ...
    try {
      ...
      const data = await res.json();
      const modelsList: string[] = Array.isArray(data.models) ? data.models : [];

      if (currentModel && !modelsList.includes(currentModel) && currentModel !== '__custom__') {
        modelsList.unshift(currentModel);
      }

      selectElem.innerHTML = modelsList.map(m => 
        `<option value="${m}" ${m === currentModel ? 'selected' : ''}>⚡ ${m}</option>`
      ).join('') + `<option value="__custom__" ${currentModel === '__custom__' ? 'selected' : ''}>✏️ Enter custom model name...</option>`;
  ...
  switchSettingsTab(tabName: string): void {
    ...
    if (systemForm) systemForm.style.display = tabName === 'system' ? 'block' : 'none';
    if (catEditor) catEditor.style.display = tabName === 'categories' ? 'flex' : 'none';
    if (decEditor) decEditor.style.display = tabName === 'decisions' ? 'flex' : 'none';
    if (btnSys) btnSys.style.borderBottom = tabName === 'system' ? '2px solid var(--accent-blue)' : 'none';
    if (btnCat) btnCat.style.borderBottom = tabName === 'categories' ? '2px solid var(--accent-blue)' : 'none';
    if (btnDec) btnDec.style.borderBottom = tabName === 'decisions' ? '2px solid var(--accent-blue)' : 'none';
  ...
  async loadDecisionsManager(): Promise<void> {
    ...
    try {
      ...
      const enabledCount = decisions.filter((d: any) => d.enabled !== 0).length;
      if (summary) {
      ...
      container.innerHTML = decisions.map((d: any, idx: number) => this.renderDecisionCard(d, idx)).join('');
    } catch (err: any) {
  ...
  renderDecisionCard(d: any, idx: number): string {
    const state = this.app.state;
    const esc = (v: any) => state.escapeHtml(v == null ? '' : String(v));
    const id = d.id != null ? d.id : idx;
    const keywords = Array.isArray(d.rule_keywords) ? d.rule_keywords.filter((k: string) => k.trim()) : [];
    ...
    const badge = (text: string, color: string) =>
      `<span style="font-size:0.72rem; padding:0.1rem 0.5rem; border-radius:6px; border:1px solid; background:rgba(15,23,42,0.6); color:${color}; border-color:${color}55; white-space:nowrap;">${text}</span>`;
      ...
      <div id="decisionCard_${id}" style="background: #111827; border: 1px solid ${enabled ? '#1f2937' : '#1f2937'}; border-left: 3px solid ${enabled ? '#38bdf8' : '#64748b'}; border-radius: 8px; padding: 0.8rem 1rem;">
        ...
        <div style="display: flex; align-items: center; gap: 0.4rem; margin-top: 0.45rem; flex-wrap: wrap;">
          ...
          ${keywords.length === 0
            ...
            : keywords.map((k: string) => `<span style="font-size: 0.75rem; padding: 0.12rem 0.5rem; border-radius: 12px; background: rgba(56,189,248,0.12); border: 1px solid rgba(56,189,248,0.35); color: #7dd3fc;">${esc(k)}</span>`).join('')}
        </div>
        ${snippet ? `
  ...
  toggleDecisionEdit(id: number): void {
    const d = this._decisions.find((x: any) => x.id === id);
  ...
  async saveDecision(id: number): Promise<void> {
    const category = (document.getElementById(`decCategory_${id}`) as HTMLInputElement | null)?.value.trim() || '';
    const subcategory = (document.getElementById(`decSubcategory_${id}`) as HTMLInputElement | null)?.value.trim() || '';
    const keywordsRaw = (document.getElementById(`decKeywords_${id}`) as HTMLInputElement | null)?.value || '';
    const reason = (document.getElementById(`decReason_${id}`) as HTMLTextAreaElement | null)?.value || '';
    const enabled = (document.getElementById(`decEnabled_${id}`) as HTMLInputElement | null)?.checked || false;
    ...
    try {
      const res = await fetch(`/api/manual-decisions/${id}`, {
        ...
        body: JSON.stringify({
          ...
          rule_keywords: keywordsRaw.split(',').map((k: string) => k.trim()).filter(Boolean),
          enabled
        })
      });
      const data = await res.json();
  ...
  async setDecisionEnabled(id: number, enabled: boolean): Promise<void> {
    try {
      const res = await fetch(`/api/manual-decisions/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async deleteDecision(id: number): Promise<void> {
    ...
    if (!confirm(`Delete this decision for '${label}'?\n\nIt will stop teaching the AI entirely. The document and its file are NOT affected.`)) return;
    try {
  ...
  async clearAllDecisions(): Promise<void> {
    if (!confirm('Delete ALL recorded human decisions?\n\nThe AI will stop learning from every past move. This cannot be undone (documents and files are NOT affected).')) return;
    try {
  ...
  async handleSaveSettings(e: any): Promise<void> {
    e.preventDefault();
    const state = this.app.state;
  ...
  renderCategoriesManager(): void {
    ...
    const visibleCategories = state.categories.filter((cat: any) => {
      if (!state.activeManageCatId) return true;
    ...
    visibleCategories.forEach((cat: any) => {
      ...
      const catIdx = state.categories.findIndex((c: any) => c.id === cat.id);
      if (catIdx === -1) return;
  ...
  saveSingleCategory(catIdx: number): void {
    const state = this.app.state;
    const cat = state.categories[catIdx];
    if (!cat) return;

    const nameVal = state.getVal(`catName_${catIdx}`).trim();
    const aliasesVal = state.getVal(`catAliases_${catIdx}`).split(',').map((a: string) => a.trim()).filter(Boolean);
  ...
  saveSingleSubcategory(catIdx: number, subIdx: number): void {
    const state = this.app.state;
    const cat = state.categories[catIdx];
    ...
    const aliasesVal = state.getVal(`subAliases_${catIdx}_${subIdx}`).split(',').map((a: string) => a.trim()).filter(Boolean);
  ...
  deleteSubcategory(catIdx: number, subIdx: number): void {
    const state = this.app.state;
    const cat = state.categories[catIdx];
    if (!cat || !cat.subcategories || !cat.subcategories[subIdx]) return;
  ...
  addNewSubcategory(catIdx: number): void {
    const state = this.app.state;
    const cat = state.categories[catIdx];
    if (!cat) return;
    ...
    const aliasesVal = state.getVal(`newSubAliases_${catIdx}`).split(',').map((a: string) => a.trim()).filter(Boolean);
    ...
    if (cat.subcategories.some((s: any) => s.id === idVal)) {
  ...
  async handleAddCategory(): Promise<void> {
    ...
    const aliases = state.getVal('newCatAliases').split(',').map((a: string) => a.trim()).filter(Boolean);
    ...
    if (state.categories.some((c: any) => c.id === id)) {
  ...
  async syncCategoriesToServer(): Promise<void> {
    const state = this.app.state;
    try {
...
(window as any).ModalsManager = ModalsManager;
