...
class TriageEventsManager {
  ...
  async checkActiveTaskStatus(): Promise<void> {
    try {
  ...
  updateGlobalOperationUI(taskState: any): void {
    ...
    const btnScan = document.getElementById('btnScan') as HTMLButtonElement | null;
    const btnRepair = document.getElementById('btnRepair') as HTMLButtonElement | null;
    const btnClear = document.getElementById('btnClear') as HTMLButtonElement | null;

    if (!banner || !taskState) return;

    if (taskState.isRunning) {
      banner.classList.remove('hidden');

      let label = 'WORKING...';
      if (taskState.type === 'REPAIR') label = '🛠️ REPAIRING REGISTRY & RELOCALIZING...';
      else if (taskState.type === 'SCAN') label = '🔍 SCANNING & TRIAGING FILES...';
      else if (taskState.type === 'CLEAR') label = '🧹 CLEARING REGISTRY & RETURNING FILES...';

      if (typeLabel) typeLabel.textContent = label;
      if (counterLabel) counterLabel.textContent = `${taskState.processedFiles || 0} / ${taskState.totalFiles || 0} files (${taskState.percent || 0}%)`;
      if (progressFill) progressFill.style.width = `${taskState.percent || 0}%`;
      if (currentStage) currentStage.textContent = `Stage: ${taskState.stage || 'Processing'}`;
      if (currentFile) currentFile.textContent = taskState.currentFile ? `📄 ${taskState.currentFile}` : (taskState.message || '');
      ...
      if (btnRepair) { btnRepair.disabled = false; btnRepair.classList.remove('btn-op-active'); btnRepair.textContent = '🔧 Repair Registry'; }
      if (btnClear) { btnClear.disabled = false; }
    }
  }

  setupGlobalTriageSSE(): void {
    if (!!(window as any).EventSource) {
  ...
  async handleStartOllama(): Promise<void> {
  ...
  openInChrome(target?: string | number): void {
    if (!target) return;
    let fileUrl = '';
  ...
  async handleOpenRaws(): Promise<void> {
    try {
  ...
  async handleOpenArchive(): Promise<void> {
    try {
      const res = await fetch('/api/config');
      const cfg = await res.json();
  ...
  async handleRepairRegistry(): Promise<void> {
    const btn = document.getElementById('btnRepair') as HTMLButtonElement | null;
    if (btn) {
      btn.disabled = true;
  ...
  async handleScan(): Promise<void> {
    ...
    const btnClose = document.getElementById('btnCloseScanProgress') as HTMLButtonElement | null;
    const btnDone = document.getElementById('btnDoneScanProgress') as HTMLButtonElement | null;
      ...
      sse.onmessage = e => {
        try {
          ...
          if (evt.type === 'SCAN_STARTED') {
            ...
            (evt.files || []).forEach((fname: string) => {
  ...
  async handleUnlockOp(): Promise<void> {
    if (!confirm('Forcefully clear active operation locks and unlock the system?')) return;
    try {
  ...
  exportDocumentsCsv(): void {
    ...
    if (state.activeCategory) url += `category=${encodeURIComponent(state.activeCategory)}&`;
    if (state.activeSubcategory) url += `subcategory=${encodeURIComponent(state.activeSubcategory)}&`;
...
(window as any).TriageEventsManager = TriageEventsManager;
