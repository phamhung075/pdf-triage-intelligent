...
class ChatAssistantManager {
  ...
  init(): void {
    ...
    const form = document.getElementById('chatInputForm') as HTMLFormElement | null;

    if (btnOpen) btnOpen.addEventListener('click', () => this.openChatModal());
    if (btnClose) btnClose.addEventListener('click', () => this.closeChatModal());

    if (form) {
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const input = document.getElementById('chatInputText') as HTMLInputElement | null;
  ...
  async updateMcpStatusBadge(): Promise<void> {
  ...
  appendMessageBubble(role: 'user' | 'assistant', text: string, matchedDocs: any[] = []): void {
    const historyContainer = document.getElementById('chatMessageHistory');
    if (!historyContainer) return;
  ...
  async openDocFolder(docId: number): Promise<void> {
    try {
...
(window as any).ChatAssistantManager = ChatAssistantManager;
