...
class CategoryPillsManager {
        ...
        try {
            try {
                ...
                if (cfgRes.ok) {
                    ...
                    if (cfg.language)
                        state.systemLanguage = cfg.language;
                }
            }
            catch (err) { }
            ...
            if (statDocs)
                statDocs.textContent = totalDocsCount;
            if (statCats) {
                const activeCats = state.categories.filter((c) => (c.count || 0) > 0).length;
            ...
            if (filterCatSelect) {
                ...
                const validCats = state.categories.filter((c) => (c.count || 0) > 0 || c.id === currVal);
                filterCatSelect.innerHTML = `<option value="">${allCatLabel}</option>` + validCats.map((c) => `<option value="${c.id}" ${c.id === currVal ? 'selected' : ''}>${state.getLocalizedName(c)} (${c.count || 0})</option>`).join('');
            }
    ...
    updateFilterSubcategoriesDropdown() {
        ...
        const catObj = catList.find((c) => c.id === state.activeCategory);
        const subs = (catObj && catObj.subcategories) ? catObj.subcategories : [];
        filterSubSelect.innerHTML = '<option value="">All Subcategories</option>' + subs.map((s) => `<option value="${s.id}" ${s.id === state.activeSubcategory ? 'selected' : ''}>${s.name || s.id} (${s.count || 0})</option>`).join('');
    }
    renderSubcategories() {
        const state = this.app.state;
        ...
        if (state.activeCategory) {
            const catObj = state.categories.find((c) => c.id.toLowerCase() === state.activeCategory.toLowerCase());
            ...
            state.allLoadedDocs.forEach((doc) => {
                if (doc.category.toLowerCase() === state.activeCategory.toLowerCase()) {
                    ...
                    if (subId && subId !== 'general' && !subcatsMap.has(subId)) {
                        const matchingDocs = state.allLoadedDocs.filter((d) => d.category.toLowerCase() === state.activeCategory.toLowerCase() && (d.subcategory || '').toLowerCase() === subId);
            ...
            state.allLoadedDocs.forEach((doc) => {
                const subId = (doc.subcategory || '').toLowerCase();
                if (subId && subId !== 'general' && !subcatsMap.has(subId)) {
                    const matchingDocs = state.allLoadedDocs.filter((d) => (d.subcategory || '').toLowerCase() === subId);
                    subcatsMap.set(subId, {
                        name: state.formatSubName(subId),
                        count: matchingDocs.length
                    });
    ...
    filterByCategoryAndSubcategory(catId, subId) {
        if (!catId)
            return;
        const state = this.app.state;
        state.activeCategory = catId.toLowerCase();
    ...
    filterBySearchOrSubcategory(tagValue, catId, subId) {
...
window.CategoryPillsManager = CategoryPillsManager;
