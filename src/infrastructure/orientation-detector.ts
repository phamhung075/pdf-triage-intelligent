...
// Tesseract's Orientation & Script Detection (measures actual text-pixel orientation — the most
// reliable single signal here, but slower, so it only runs when needed). When EXIF and the model
// agree, that shared answer is used directly with no OCR pass. Otherwise — including when EXIF
// is absent, since there's then nothing for the model to agree with — Tesseract OSD breaks the
// tie; if OSD itself is inconclusive, the model's answer is the final fallback.
